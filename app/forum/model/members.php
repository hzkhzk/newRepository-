<?php
class forum_mdl_members extends dbeav_model {
...
/*
 * （b3c_members）编号，会员等级，(pam_members)名字，(forum_)主题，主题类型，
 */
	/*public function count($filter=null){
	    //自己写sql，返回满足条件的条数
	    $sql = 'SELECT count(*) as _count FROM (SELECT I.goods_id FROM '.
	        kernel::database()->prefix.'forum_orders as O LEFT JOIN '.
	        kernel::database()->prefix.'forum_order_items as I ON O.order_id=I.order_id LEFT JOIN '.
	        kernel::database()->prefix.'forum_goods as G ON G.goods_id=I.goods_id WHERE '.
...
/*      
   public function getlist($cols='*', $filter=array(), $offset=0, $limit=-1, $orderType=null){
     $model=app::get('b2c')->model('members');
      $rows= $model->getList('member_id');
